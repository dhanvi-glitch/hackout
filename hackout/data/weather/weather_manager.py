...
class WeatherManager:
    ...
    def invalidate_cache(self):
        """Flushes local cache files when switching geographic location."""
        try:
            if self.forecast_cache_file.exists():
                self.forecast_cache_file.unlink(missing_ok=True)
            if self.current_cache_file.exists():
                self.current_cache_file.unlink(missing_ok=True)
            logger.info("WeatherManager cache invalidated successfully.")
        except Exception as e:
            logger.warning(f"Error clearing weather cache: {e}")
    ...
    def _load_forecast_cache(
        ...
        try:
            ...
            # Prevent stale cross-location cache: check coordinates match within 0.05 deg
            if latitude is not None and "latitude" in data:
                if abs(float(data["latitude"]) - float(latitude)) > 0.05:
                    logger.info("Cached forecast coordinates mismatch; ignoring stale cache.")
                    return None
            if longitude is not None and "longitude" in data:
                if abs(float(data["longitude"]) - float(longitude)) > 0.05:
                    logger.info("Cached forecast coordinates mismatch; ignoring stale cache.")
                    return None
    ...
    def _save_current_cache(
        ...
        try:
            ...
            logger.warning(f"Failed to save current weather cache: {e}")
    ...
    def _load_current_cache(
        ...
        try:
            ...
            # Prevent stale cross-location cache: check coordinates match
            if latitude is not None and "latitude" in d:
                if abs(float(d["latitude"]) - float(latitude)) > 0.05:
                    return None
            if longitude is not None and "longitude" in d:
                if abs(float(d["longitude"]) - float(longitude)) > 0.05:
                    return None
