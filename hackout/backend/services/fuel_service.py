...
class FuelService:
    ...
    def get_status(self, db: Optional[Session] = None) -> FuelStatusResponse:
        ...
        try:
            from data import calculate_fuel_autonomy
            autonomy = calculate_fuel_autonomy(fuel_remaining_l=self.fuel_remaining, daily_burn_l=self.burn_rate_daily)
            days = int(autonomy["days_remaining"])
            depletion_date = autonomy["depletion_date"]
            system_status = autonomy["status"]
        except Exception:
