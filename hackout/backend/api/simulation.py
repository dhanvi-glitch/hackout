...
async def run_simulation(req: SimulationRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Attempt high-fidelity simulation via Member 4's data simulation engine
        try:
            import data
            sim_res = data.run_simulation_api(
                scenario=sc,
                severity=req.severity or 50.0,
                duration_hours=req.durationHours or 12.0
            )
            b_raw = sim_res["before"]
            a_raw = sim_res["after"]
            deltas = sim_res.get("deltas", {})
            met = sim_res.get("metrics", {})

            before = {
                "solar": round(float(b_raw.get("solar_avg_kw", 30.0)), 1),
                "wind": round(float(b_raw.get("wind_avg_kw", 15.0)), 1),
                "battery": round(float(b_raw.get("battery_avg_kw", b_raw.get("battery_soc_end", 5.0))), 1),
                "diesel": round(float(b_raw.get("diesel_avg_kw", 0.0)), 1),
                "demand": round(float(b_raw.get("demand_avg_kw", 50.0)), 1),
            }
            after = {
                "solar": round(float(a_raw.get("solar_avg_kw", 0.0)), 1),
                "wind": round(float(a_raw.get("wind_avg_kw", 0.0)), 1),
                "battery": round(float(a_raw.get("battery_avg_kw", a_raw.get("battery_soc_end", 0.0))), 1),
                "diesel": round(float(a_raw.get("diesel_avg_kw", 0.0)), 1),
                "demand": round(float(a_raw.get("demand_avg_kw", 50.0)), 1),
            }
            p0 = round(float(met.get("p0_served_pct", 100.0)), 1)
            p1 = round(float(met.get("p1_served_pct", 92.0)), 1)
            p2 = round(float(met.get("p2_served_pct", 61.0)), 1)
            add_diesel = round(float(deltas.get("additional_diesel_liters", 0.0)), 1)
            add_co2 = round(float(deltas.get("additional_co2_kg", 0.0)), 1)
            cost_diff = f"{round(float(deltas.get('cost_difference_dollars', 0.0)), 2):.2f}"
            used_engine = True
        except Exception:
            used_engine = False
