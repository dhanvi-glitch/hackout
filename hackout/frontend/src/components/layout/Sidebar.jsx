...
export const Sidebar = () => {
  ...
  const loc = activeLocation || DEMO_LOCATION;
  return (
    <aside className="w-64 bg-[#131B29] border-r border-slate-800 flex flex-col justify-between shrink-0 h-screen sticky top-0 z-30">
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="bg-slate-900/80 rounded-lg p-3 border border-slate-800/80">
          ...
          <p className="text-[11px] font-medium text-slate-300 truncate" title={loc.community || loc.name}>
            {loc.community || loc.name}
          </p>
          <p className="text-[10px] text-slate-500 font-mono mt-0.5 truncate">
            {loc.shortName || loc.village}, {loc.state}
          </p>
        </div>
      </div>
    </aside>
